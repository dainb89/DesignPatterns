...
public class Program
{
    public static void Main()
    {
		...
		string tOrd = "1 Espresso with whip and steamed Milk";
		Console.WriteLine("Order =>: " + tOrd);
        Console.WriteLine(cStore.OrderCoffe(tOrd));
		tOrd = "decaf with 2 Steamed milk";
		Console.WriteLine("Order =>: " + tOrd);
        Console.WriteLine(cStore.OrderCoffe(tOrd));
		tOrd = "venti Decaf with 2 Steamed milk";
		Console.WriteLine("Order =>: " + tOrd);
		Console.WriteLine(cStore.OrderCoffe(tOrd));
		tOrd = "dark Roast with 2 mocha and 1 soy";
		Console.WriteLine("Order =>: " + tOrd);
        Console.WriteLine(cStore.OrderCoffe(tOrd));
		tOrd = "1 tall Dark Roast with 2 mocha and 1 soy";
		Console.WriteLine("Order =>: " + tOrd);
        Console.WriteLine(cStore.OrderCoffe(tOrd));
    }
}
public class CoffeStore
{
    ...
    public string OrderCoffe(string orderTxt)
    {
        ...
        for (int ii = 1; ii < arOrdTxt.Length; ii++)
        {
            ...
            if (txt.Contains("mocha")) beverage = CreateCondiment("Mocha", prTxt, beverage);
            if (txt.Contains("soy")) beverage = CreateCondiment("Soy", prTxt, beverage);
            if (txt.Contains("whip")) beverage = CreateCondiment("Whip", prTxt, beverage);
